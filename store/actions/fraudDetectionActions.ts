...
export const runFraudDetection = (currentReading: OdometerReading) => {
  return async (dispatch: Dispatch<FraudDetectionAction>, getState: () => RootState) => {
    try {
      ...
      // Dispatch alerts if any (limit to 50 alerts max for performance)
...
/**
 * Reset fraud detection session data (called on app start)
 */
export const resetFraudDetectionSession = (): FraudDetectionAction => {
  return {
    type: FRAUD_DETECTION_TYPES.RESET_FRAUD_SESSION,
  };
};
